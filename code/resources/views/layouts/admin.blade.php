    ...
    <div class="flex h-screen relative z-10" x-data="{ sidebarOpen: true, mobileMenuOpen: false }">
        ...
        >
            ...
            <nav class="p-4 space-y-1">
                ...
                <div class="mt-6">
                    <h3 x-show="sidebarOpen || window.innerWidth >= 768" class="px-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Work Opportunities</h3>

                    <a href="{{ route('admin.job-offers.index') }}" class="nav-item group flex items-center px-3 py-2 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.job-offers.*') ? 'bg-teal-50 text-teal-600 dark:bg-teal-900/50 dark:text-teal-400' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                        <svg class="w-5 h-5 mr-3 {{ request()->routeIs('admin.job-offers.*') ? 'text-teal-600 dark:text-teal-400' : 'text-slate-500 dark:text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                        <span x-show="sidebarOpen || window.innerWidth >= 768" class="font-medium">Job Offers</span>
                    </a>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <a href="{{ route('admin.skills.index') }}" class="nav-item group flex items-center px-3 py-2 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.skills.*') ? 'bg-yellow-50 text-yellow-600 dark:bg-yellow-900/50 dark:text-yellow-400' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                        <svg class="w-5 h-5 mr-3 {{ request()->routeIs('admin.skills.*') ? 'text-yellow-600 dark:text-yellow-400' : 'text-slate-500 dark:text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
                        </svg>
                        <span x-show="sidebarOpen || window.innerWidth >= 768" class="font-medium">Skills</span>
                    </a>
