...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 lg:mb-8">
        <div class="mb-4 sm:mb-0">
            ...
            <p class="text-gray-600 mt-1">Name, link and a main photo — that's all you need</p>
        </div>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <form method="POST" action="{{ route('admin.courses.store') }}" enctype="multipart/form-data" class="p-4 lg:p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-xs mt-1"><i class="fas fa-link mr-1"></i>URL: /courses/<span id="slug-preview">…</span></p>
                    @error('slug')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
